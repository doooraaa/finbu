...
export async function listTransactions({ from, to, type, categoryId, userId, linkedType, linkedId } = {}) {
  let items = from || to
  ...
  if (linkedType) items = items.filter((t) => t.linkedType === linkedType);
  if (linkedId) items = items.filter((t) => t.linkedId === linkedId);
